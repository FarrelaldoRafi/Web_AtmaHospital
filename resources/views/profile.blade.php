...
// URL gambar default dari folder public/icon
const defaultProfilePic = 'https://cdn-icons-png.flaticon.com/512/552/552721.png';

// Simpan nilai awal dari gambar profil dan input form
const initialValues = {};
...
// Tombol batal klik handler
cancelButton.addEventListener('click', function() {
    // Reset semua input dan gambar profil
    allInputs.forEach(input => {
        input.disabled = true;
        input.value = initialValues[input.name]; // Kembali ke nilai awal
    });

    // Jika file input diisi, kembali ke ikon default
    if (fileInput.value) {
        profilePic.src = defaultProfilePic; // Reset ke ikon default jika diunggah tetapi batal
    } else {
        profilePic.src = initialProfilePic; // Kembali ke gambar profil awal jika tidak ada file yang dipilih
    }
...
form.addEventListener('submit', function(e) {
    ...
    .then(data => {
        if (data.success) {
            // Update gambar profil jika ada gambar baru diupload
            if (data.image_url) {
                profilePic.src = data.image_url;
                navProfileImg.src = data.image_url;
                initialProfilePic = data.image_url; // Update initialProfilePic ke URL baru
            }

            // Update nilai awal input form
            allInputs.forEach(input => {
                initialValues[input.name] = input.value;
            });

            // Reset ke mode tampilan
...
// Inisialisasi gambar profil dari sesi jika ada
window.addEventListener('load', function() {
    ...
    if (sessionProfilePic) {
        ...
        initialProfilePic = fullUrl; // Atur initialProfilePic ke gambar yang dimuat dari sesi
    }
});
</script>
</main>
